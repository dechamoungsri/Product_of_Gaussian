...
def gen_mean_and_cov_of_dct(names, lf0_mean):
    ...
    for n in names:
        ...
        if n in syllable_dct_dict:
            ...
            # data.fill(unvoice)
            data[:,0].fill( np.mean(lf0_mean[:,0]) )
            data[:,1].fill( np.mean(lf0_mean[:,1]) )
            data[:,2].fill( np.mean(lf0_mean[:,2]) )
...
def cal_PoG(config):
    ...
    syllable_mean, syllable_cov = gen_mean_and_cov_of_dct(names, lf0_mean)

    print w.shape, syllable_mean.shape, syllable_cov.shape, lf0_mean.shape, lf0_cov.shape

    mean, cov = PoGUtility.cal_mean_variance_of_product_of_gaussian(w, syllable_mean, syllable_cov, lf0_mean, lf0_cov)
...
if __name__ == '__main__':
    ...
    num_coeff = 7
    outpath = '/work/w21/decha/Interspeech_2017/Result/03_Given_syllable_dct_use_mean_as_unvoice/num_dct_cov_{}/'.format(num_coeff)
