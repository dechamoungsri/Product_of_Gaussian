...
if __name__ == '__main__':
    ...
    training_size = 250
    num_coeff = 3
    mainoutpath = '/29_script/Exp_2_stress/'
    ...
    stress_optimal_list = [
        ...
        # {
        #     'tone' : 4,
        #     'startbeta' : 3.0, 'endbeta' : 3.1,
        #     'startgamma' : 2.0, 'endgamma' : 2.1,
        # }
         {
            'tone' : 4,
            'startbeta' : 10.0, 'endbeta' : 20.0,
            'startgamma' : 1.9, 'endgamma' : 2.0,
        }
    ]
    ...
    for config in stress_optimal_list:
        ...
        p = subprocess.call(['/usr/local/bin/python', '-u', 'run_joint_main_three_optimal.py', 
            ...
            '-file_num_start', '{}'.format(file_num_start), 
            '-file_num_end', '{}'.format(file_num_end), 
            '-plot'
            ], stdout=f)
