...
def cal_lf0(config):
    ...
    zero_mean = np.load('{}/mean.npy'.format(zero_base_path))
    zero_cov = np.load('{}/cov.npy'.format(zero_base_path))

    z_mean = zero_mean

    var = np.load('{}'.format(zero_var_path))
    zero_var = np.sum(var, axis=0)

    temp_mean = []
    for i in range( len(zero_mean[0]) ):
        temp_mean.append( zero_mean[:,i] )
    zero_mean = np.array(temp_mean)

    if weight_type_of_stress=='stress':
        zero_w = PoGUtility.generate_DCT_W_without_consonant_on_stress_vuv_head_tail_tone(len(lf0_cov), dur_list, 1, p_in_s_file, stress, vuv, tone)
    elif weight_type_of_stress=='unstress':
        zero_w = PoGUtility.generate_DCT_W_without_consonant_on_unstress_vuv_head_tail_tone(len(lf0_cov), dur_list, 1, p_in_s_file, stress, vuv, tone)
    else:
        zero_w = PoGUtility.generate_DCT_W_without_consonant_vuv_head_tail_tone(len(lf0_cov), dur_list, 1, p_in_s_file, stress, vuv, tone)

    zero_B = gamma * PoGUtility.cal_sum_of_mean_part(zero_var, zero_w, zero_cov, zero_mean)
    zero_A = gamma * PoGUtility.cal_sum_of_weight_part(zero_var, zero_w, zero_cov)

    #----------Combine Model--------#

    L = linalg.cholesky(frame_A + syl_A + zero_A, lower=True)
    lf0 = linalg.cho_solve((L, True) , frame_B + syl_B + zero_B)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-startgamma')
    parser.add_argument('-endgamma')
    ...
    parser.add_argument('-syllable_predicted_path')
    parser.add_argument('-zero_coeff_path')
    parser.add_argument('-file_num_start')
    parser.add_argument('-file_num_end')
    ...
    file_num_start = int(args.file_num_start)
    file_num_end = int(args.file_num_end)

    startgamma = float(args.startgamma)
    endgamma = float(args.endgamma)
    ...
    syllable_predicted_path = args.syllable_predicted_path
    zero_coeff_path = args.zero_coeff_path
    ...
    syllable_predicted_dct_path = '{}/a-{}/infer/a-{}/demo/seed-00/M-{}/B-{}/num_iters-5/dur/predictive_distribution/'.format(syllable_predicted_path, data_dict[training_size], data_dict[training_size], block_size, block_size)

    zero_coeff_predicted_dct_path = '{}/a-{}/infer/a-{}/demo/seed-00/M-{}/B-{}/num_iters-5/dur/predictive_distribution/'.format(zero_coeff_path, data_dict[training_size], data_dict[training_size], block_size, block_size)
    ...
    method = '/{}/Syllable_training_size_{}/block_size_{}/num_coeff_{}/tone_{}/'.format(mainoutpath, training_size, block_size, num_coeff, tone)
    ...
    for b in np.arange(start, end, increment):
        old_rmse = 100000000.0
        for g in np.arange(startgamma, endgamma, increment):
            ...
            print 'Alpha : Beta : Gamma:',  alpha,  b, g

            outbase = outname
            outpath = '{}/Alpha_{}_Beta_{}_Gamma_{}/lf0/'.format(outbase, alpha, beta, gamma)
            figure_path = '{}/Alpha_{}_Beta_{}_Gamma_{}/fig/'.format(outbase, alpha, beta, gamma)
