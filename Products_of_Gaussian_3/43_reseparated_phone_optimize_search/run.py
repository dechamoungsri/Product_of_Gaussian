...
def lf0_distortion_syn_is_gpr_format(org_path,syn_path, stress_list, mono_label, phone_type):
    ...
    for base in Utility.list_file(org_path) :
        ...
        for st, mono in zip(stress, mono_file):
            ...
            if phone_type == 'no':
                ...
                if phone_type == 'vowel':
                    pt = '1'
                elif phone_type == 'initial':
                    pt = '0'
                elif phone_type == 'final':
                    pt = '2'

                if (st[0] == str(stress_type)) & ( st[1] == '{}'.format(tone) ) & ( str(st[2]) == pt ):
                        stress_index = np.append(stress_index, np.arange(start, end), axis=0 )
        ...
        synthesis_vector = syn_path[Utility.get_basefilename(base)]
...
def cal_lf0(config):
    ...
    ph_A_1toN, ph_B_1toN = JointModule.cal_suprasegmental('phone', config['phone_level_config'],  coeff_end_position=num_coeff, coeff_start_position=1, use_consonant=False, separated_phone=phone_type)
    ...
    ph_A_0, ph_B_0 = JointModule.cal_suprasegmental('phone', config['phone_level_zero_config'],  coeff_end_position=1, use_consonant=False, separated_phone=phone_type)
...
def run_cal_optimal_and_rmse(opt, outname, main_out_data, org_for_distortion, stress_list, mono_label, phone_type, is_save_object=False):
    ...
    rmse = lf0_distortion_syn_is_gpr_format(org_for_distortion, syn_result, stress_list, mono_label, phone_type)

    if is_save_object:
        now = datetime.datetime.now()
        Utility.save_obj(syn_result, '{}/lf0_generated_by_{}_{}_{}_{}_{}_time_{}.pkl'.format(
...
if __name__ == '__main__':
    ...
    parser.add_argument('-phone_type', required=True)
    parser.add_argument('-is_save_object', action='store_true', default=False)
    ...
    is_save_object = args.is_save_object
    ...
    syllable_predicted_dct_path = '{}/out_{}/tsc/a-{}/infer/a-{}/demo/seed-00/M-{}/B-{}/num_iters-5/dur/predictive_distribution/'.format(syllable_predicted_basepath, 'all', data_dict[training_size], data_dict[training_size], block_size, block_size)

    zero_coeff_predicted_dct_path = '{}/out_{}/tsc/a-{}/infer/a-{}/demo/seed-00/M-{}/B-{}/num_iters-5/dur/predictive_distribution/'.format(zero_coeff_predicted_basepath, 'all', data_dict[training_size], data_dict[training_size], block_size, block_size)
    ...
    st = np.array([1.0, start_ph_1toN, start_ph_0, start_syl_1toN, start_syl_0])
    ed = np.array([1.0, end_ph_1toN, end_ph_0, end_syl_1toN, end_syl_0])
    ...
    # rmse = run_cal_optimal_and_rmse(optimal_list[0], outname, main_out_data, org_for_distortion, stress_list, mono_label, phone_type)

    # Parent, opt
    optimal_queue = [ ('', st) ]
    rmse_list = dict()
    while len(optimal_queue) != 0:

        print strftime("%Y-%m-%d %H:%M:%S", gmtime())

        opt_pop = optimal_queue.pop()

        rmse = run_cal_optimal_and_rmse(opt_pop[1], outname, main_out_data, org_for_distortion, stress_list, mono_label, phone_type, is_save_object=is_save_object)
        ...
        if opt_pop[0] != '':
            parent = opt_pop[0]
            parent_rmse = rmse_list[parent]
            if rmse > parent_rmse: 
                continue

        for idx, opt in enumerate(opt_pop[1]):
            base_opt = np.copy(opt_pop[1])

            if base_opt[idx] >= ed[idx]: continue

            base_opt[idx] = base_opt[idx] + increment
    ...
    print tups
    Utility.save_obj(tups, '{}/results_{}.pkl'.format(
        outname, 
    ...
    #     for diff in [st[idx], ed[idx], (st[idx]+ed[idx])/2 ]:

    #         opt = np.copy(st)
    #         opt[idx] = diff

    #         rmse = run_cal_optimal_and_rmse(opt, outname, main_out_data, org_for_distortion, stress_list, mono_label, phone_type)
    #         rmse_list.append( (rmse, opt) )

    #     while True:
            
    #         Utility.sort_by_index(rmse_list, 0)
    #         first = rmse_list[0][1]
    #         second = rmse_list[1][1]

    #         rank1 = rmse_list[0]
    #         rank2 = rmse_list[1]

    #         # print 'Opt :', first, second
    #         # print 'Abs :', abs(first[idx] - second[idx])

    #         if abs(first[idx] - second[idx]) < threshold: 
    #             optimal_threshold.append(first[idx])
    #             break

    #         rmse_list = []
    #         rmse_list.append(rank1)
    #         rmse_list.append(rank2)

    #         diff = (first[idx]+second[idx])/2
    ...
    # rmse = run_cal_optimal_and_rmse(optimal_threshold, outname, main_out_data, org_for_distortion, stress_list, mono_label, phone_type)
